                ...
                <div className="grid grid-cols-2 gap-3">
                  {Object.values(ShiftType).filter(v => v !== ShiftType.OFF).map(shift => (
                    <button
                      key={shift}
                      type="button"
                      onClick={() => setFormData({...formData, assignedShift: shift})}
                      className={`flex items-center justify-center gap-2 py-3 rounded-xl text-xs font-black transition-all border ${
                        formData.assignedShift === shift 
